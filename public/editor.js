...
불법복제 및 카피 시 엄중한 법적처벌을 받을 수 있음을 고지합니다.</div>
  ...
  bucketmb:function(){return '<div class="sec-wrap s-bucket s-bucket-mb">'+bucketInnerMb()+'</div>';},
...
function bucketInnerMb(){return '<div class="ss-theme sec-toolbar"><span>🎨 테마</span><input type="color" value="#f2f2f2" onchange="bucketTheme(this)" title="배경 색상"></div>'+'<div class="ss-mb-hero"><div class="ss-box ss-ar-mhero">'+izNew('히어로 (모바일)','750 × 1110px','')+'<div class="ss-hero-ov"></div></div><div class="ss-hero-txt ss-movable"><div class="ss-tag" contenteditable>BRAND</div><div class="ss-h1" contenteditable>25 SPRING<br>COLLECTION</div><div class="ss-sub" contenteditable>SPECIAL BENEFIT 00% COUPON</div></div></div>'+'<div class="ss-mb-golf"><div class="ss-box ss-ar-mgolf">'+izNew('스프링 골프','540 × 760px','')+'</div><div class="ss-golf-txt"><div class="ss-h2" contenteditable>FOR<br>SPRING GOLF</div><div class="ss-tag dark" contenteditable>BRAND</div><div class="ss-body" contenteditable>모던한 컬러감을 기본으로 과감한 패턴과 디테일,<br>다양한 소재를 믹스한 봄 스타일링 제안!</div></div></div>'+'<div class="ss-mb-big"><div class="ss-box ss-ar-mbig">'+izNew('대형 이미지','750 × 1000px','')+'</div><div class="ss-wm ss-movable" contenteditable>MASTER BUNNY<br>EDITION</div><div class="ss-box ss-ar-msq ss-mb-float">'+izNew('정사각 이미지','560 × 560px','')+'</div><div class="ss-body2" contenteditable>필드에서 기분 좋은 자신감을 안겨주는 S/S 스타일을<br>직접 만나보세요.</div></div>'+'<div class="ss-mb-mid"><div class="ss-box ss-ar-mmid">'+izNew('중간 풀폭','750 × 680px','')+'</div></div>'+'<div class="ss-mb-duo"><div class="ss-duo-l"><div class="ss-box ss-ar-mduo">'+izNew('하단 좌','360 × 480px','')+'</div><div class="ss-box ss-ar-mduo">'+izNew('하단 우','360 × 480px','')+'</div></div><div class="ss-tag dark" contenteditable>BRAND</div></div>'+'<div class="ss-mb-foot"><div class="ss-box ss-ar-mfoot">'+izNew('하단 풀폭','750 × 760px','')+'</div></div>';}
function bucketInner(){return '<div class="ss-theme sec-toolbar"><span>🎨 테마</span><input type="color" value="#f2f2f2" onchange="bucketTheme(this)" title="배경 색상"></div>'+'<div class="ss-hero"><div class="ss-hero-left"><div class="ss-box ss-ar-hero1">'+izNew('히어로 메인','940 × 1040px','')+'<div class="ss-hero-ov"></div></div><div class="ss-hero-txt ss-movable"><div class="ss-tag" contenteditable>BRAND</div><div class="ss-h1" contenteditable>25 SPRING<br>COLLECTION</div><div class="ss-sub" contenteditable>SPECIAL BENEFIT 00% COUPON</div></div></div><div class="ss-hero-right"><div class="ss-box ss-ar-hero2">'+izNew('히어로 우측','980 × 1300px','')+'</div></div></div>'+'<div class="ss-golf"><div class="ss-golf-img"><div class="ss-box ss-ar-golf">'+izNew('스프링 골프','690 × 900px','')+'</div></div><div class="ss-golf-txt"><div class="ss-h2" contenteditable>FOR<br>SPRING GOLF</div><div class="ss-tag dark" contenteditable>BRAND</div><div class="ss-body" contenteditable>모던한 컬러감을 기본으로 과감한 패턴과 디테일,<br>다양한 소재를 믹스한 봄 스타일링 제안!</div></div></div>'+'<div class="ss-big"><div class="ss-big-main"><div class="ss-box ss-ar-big">'+izNew('대형 이미지','1280 × 1100px','')+'</div><div class="ss-wm ss-movable" contenteditable>MASTER BUNNY<br>EDITION</div></div><div class="ss-big-side"><div class="ss-body2" contenteditable>필드에서 기분 좋은 자신감을 안겨주는 S/S 스타일을<br>직접 만나보세요.</div><div class="ss-box ss-ar-sq">'+izNew('정사각 이미지','730 × 730px','')+'</div></div></div>'+'<div class="ss-duo"><div class="ss-duo-l"><div class="ss-box ss-ar-duo">'+izNew('하단 좌','430 × 560px','')+'</div><div class="ss-box ss-ar-duo">'+izNew('하단 우','430 × 560px','')+'</div></div><div class="ss-tag dark" contenteditable>BRAND</div></div>'+'<div class="ss-foot"><div class="ss-box ss-ar-foot">'+izNew('하단 풀폭','1280 × 760px','')+'</div></div>';}
const SEC_META = {
...
// 전역 상태 (var - 중복 선언 허용)
var _uid=0,_jpgScale=2,_ftEl=null,_epEl=null,_slotBusy=false;
...
function setW(w,btn){document.getElementById('preview').style.width=w+'px';document.querySelectorAll('.wb-btn').forEach(function(b){b.classList.remove('act');});if(btn)btn.classList.add('act');if(typeof _fitPreviewMobile==='function')_fitPreviewMobile();}
...
function setJpgScale(s,btn){_jpgScale=s;document.querySelectorAll('#jpg-2x,#jpg-3x').forEach(function(b){b.classList.remove('act');});if(btn)btn.classList.add('act');}
...
async function saveJPG(){await doSave(_jpgScale||2,'jpg');}
...
async function doSave(scale, fmt){
  ...
  if(API){
    ...
    try{
      ...
      var _pvc = pv.cloneNode(true); _pvc.style.width = SAVE_W + 'px'; const fullHtml = html + _pvc.outerHTML + '</body></html>';

      console.log('[DEBUG] Sending - width:', SAVE_W, 'scale:', s, 'output:', SAVE_W * s, 'px');

      const res = await fetch(API + '/api/capture', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ html: fullHtml, width: SAVE_W, scale: s, format: fmt === 'jpg' ? 'jpeg' : 'png', quality: 98 })
      });
...
function addSection(type){
  ...
  if(typeof bindMovable==='function')bindMovable(sec);
...
async function saveSplit(targetW, scale, maxH, format){
  ...
  try{
    ...
    for(let ci = 0; ci < chunks.length; ci++){
      ...
      results.push({ dataUrl: final.toDataURL(format==='png'?'image/png':'image/jpeg', 0.98), w, h, idx: ci+1, total: chunks.length });
    }

    // ── 4. 갤러리 모달로 표시 ────────────────────────────────────────────
    showSplitGallery(results, targetW, scale, format);
...
function showSplitGallery(parts, targetW, scale, format){
  ...
  parts.forEach(function(part){
    ...
    dlBtn.download = 'detail-part' + part.idx + (format==='png'?'.png':'.jpg');
...
async function tplSaveCurrent(){
  ...
  showHint('⏳ 저장 중...');
  var arr=tplList().filter(function(x){return x.name!==name;});arr.unshift(snap);var ok=tplSaveAll(arr);
  if(ok){
...
function tplExportByRow(btn){ var t=_tplFromRow(btn); if(t) _downloadTpl(t); }
async function tplDeleteByRow(btn){
  var t=_tplFromRow(btn); if(!t) return;
  if(!confirm('"'+t.name+'" 삭제할까요?')) return;
  var dl=tplList().filter(function(x){return x.name!==t.name;});tplSaveAll(dl);
  renderTplList();
}
...
function makeMovable(el){
  if(!el||el.dataset.mov)return; el.dataset.mov='1';
  var h=document.createElement('div'); h.className='ss-move-handle'; h.textContent='✥'; h.title='드래그하여 이동';
  el.appendChild(h);
  h.addEventListener('mousedown',function(e){
    e.preventDefault(); e.stopPropagation();
    var par=el.parentElement; if(!par)return;
    var pr=par.getBoundingClientRect(), er=el.getBoundingClientRect();
    var offX=e.clientX-er.left, offY=e.clientY-er.top;
    el.style.left=(er.left-pr.left)+'px'; el.style.top=(er.top-pr.top)+'px'; el.style.bottom='auto'; el.style.right='auto';
    function mm(ev){ el.style.left=(ev.clientX-pr.left-offX)+'px'; el.style.top=(ev.clientY-pr.top-offY)+'px'; }
    function mu(){ document.removeEventListener('mousemove',mm); document.removeEventListener('mouseup',mu); }
    document.addEventListener('mousemove',mm); document.addEventListener('mouseup',mu);
  });
}
function bindMovable(scope){ (scope||document).querySelectorAll('.ss-movable').forEach(makeMovable); }

function bucketTheme(input){
  var sec=input.closest('.s-bucket'); if(!sec)return;
  var col=input.value; sec.style.setProperty('--bk-bg',col);
  var c=col.replace('#',''); var r=parseInt(c.substr(0,2),16),g=parseInt(c.substr(2,2),16),bl=parseInt(c.substr(4,2),16);
  var lum=(0.299*r+0.587*g+0.114*bl)/255;
  if(lum<0.55){ sec.style.setProperty('--bk-h','#ffffff'); sec.style.setProperty('--bk-body','rgba(255,255,255,.78)'); sec.style.setProperty('--bk-box','rgba(255,255,255,.16)'); sec.style.setProperty('--bk-line','rgba(255,255,255,.25)'); }
  else { sec.style.setProperty('--bk-h','#222222'); sec.style.setProperty('--bk-body','#666666'); sec.style.setProperty('--bk-box','rgba(0,0,0,.10)'); sec.style.setProperty('--bk-line','rgba(0,0,0,.10)'); }
  showHint('🎨 테마 색상 적용');
}

function bucketNew(ver){
  var nm=(ver==='mb'?'모바일':'PC');
  if(!confirm('현재 작업이 사라지고 버킷스토어 템플릿('+nm+')으로 새로 시작합니다. 저장 안 한 변경은 복구 불가. 진행할까요?')) return;
  var preview=document.getElementById('preview'); if(!preview) return;
  preview.innerHTML='';
  try{ addSection(ver==='mb'?'bucketmb':'bucketpc'); }catch(err){ console.error('bucketNew',err); }
  if(typeof closeTplModal==='function')closeTplModal();
  showHint('🆕 버킷스토어 템플릿('+nm+') 시작');
}
...
function _fitPreviewMobile(){var pv=document.getElementById('preview'),c=document.getElementById('center');if(!pv||!c)return;if(window.innerWidth<=820){pv.style.transformOrigin='top center';var pw=pv.offsetWidth||860,avail=c.clientWidth-16,sc=avail/pw;if(sc>1)sc=1;pv.style.transform='scale('+sc+')';pv.style.marginBottom=(pv.offsetHeight*(sc-1))+'px';}else{pv.style.transform='';pv.style.transformOrigin='';pv.style.marginBottom='';}}
window.addEventListener('resize',_fitPreviewMobile);
window.addEventListener('load',function(){setTimeout(_fitPreviewMobile,300);});

function _downloadTpl(t){if(!t){alert('템플릿 없음');return;}var b=new Blob([JSON.stringify(t,null,2)],{type:'application/json;charset=utf-8'});var u=URL.createObjectURL(b);var a=document.createElement('a');a.href=u;a.download='template-'+((t.name||'무제').replace(/[^a-z0-9가-힣_-]/gi,'_'))+'.json';document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(u);}
function tplDownloadCurrent(){var input=document.getElementById('tpl-name-input');var name=(input&&input.value||'').trim()||'무제';var snap=tplSnapshot(name);if(!snap){alert('내보낼 내용이 없습니다');return;}_downloadTpl(snap);showHint('✅ 현재 상태 JSON 다운로드');}
