...
// ?œí”Œë¦??€??app.post('/api/templates/save', async (req, res) => {
  try{
    const tpl = req.body;
    ...
    if(r.status >= 400) throw new Error('?€???¤íŒ¨: '+r.body);
    res.json({ ok: true });
...
app.get('/api/templates/:name', async (req, res) => {
  try{
    ...
    if(r.status >= 400) return res.status(404).json({ error: '?†ìŒ' });
